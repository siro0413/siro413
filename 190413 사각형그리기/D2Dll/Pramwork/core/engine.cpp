...
Engine::Engine()
{
	...
	*/
	...
	//Create VerTex Data
	{
		vertices = new VertexColor[6]; //배열이기때문에 DELETE_ARRAY
		...
		vertices[3].color = D3DXCOLOR{ 0.0f,0.0f,1.0f,1.0f }; //불투명하지않으니까 a값은 1

		vertices[4].position = D3DXVECTOR3{ -0.5f,+0.5f,0.0f };
		vertices[4].color = D3DXCOLOR{ 0.0f,1.0f,0.0f,1.0f }; //불투명하지않으니까 a값은 1

		vertices[5].position = D3DXVECTOR3{ +0.5f,-0.5f,0.0f };
		vertices[5].color = D3DXCOLOR{ 1.0f,0.0f,1.0f,1.0f }; //불투명하지않으니까 a값은 1
	...
	//Create ImputLayout
	{
		...
		D3D11_INPUT_ELEMENT_DESC layoutDescs[]=
		{
			...
			0,
			24, //position 에서 12바이트를 사용했기떄문에 12바이트
			D3D11_INPUT_PER_VERTEX_DATA,
			0 
			}
		};
...
void Engine::Render()
{
	...
	graphics->BeginScene();
	{
		...
		graphics->GetDeviceContex()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
		graphics->GetDeviceContex()->VSSetShader(vertexShader, nullptr, 0);
		graphics->GetDeviceContex()->PSSetShader(pixelShader, nullptr, 0);
		
		graphics->GetDeviceContex()->Draw(6, 0);
